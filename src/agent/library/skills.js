...
const blockPlaceDelay =
	settings.block_place_delay == null ? 0 : settings.block_place_delay;
...
export async function craftRecipe(bot, itemName, num = 1) {
	...
	if (!recipes || recipes.length === 0) {
		log(
			bot,
			`You do not have the resources to craft a ${itemName}. It requires: ${Object.entries(
				mc.getItemCraftingRecipes(itemName)[0][0],
			)
	...
	const craftLimit = mc.calculateLimitingResource(
		inventory,
		requiredIngredients,
	);
	...
	if (craftLimit.num < num)
		...
		log(
			bot,
			`Successfully crafted ${itemName}, you now have ${world.getInventoryCounts(bot)[itemName]} ${itemName}.`,
		);
	if (placedTable) {
		await collectBlock(bot, "crafting_table", 1);
	}
...
export async function smeltItem(bot, itemName, num = 1) {
	...
	if (
		...
		log(
			bot,
			`The furnace is currently smelting ${mc.getItemName(input_item.type)}.`,
		);
	...
	if (!furnace.fuelItem()) {
		...
		if (!fuel) {
			log(
				bot,
				`You have no fuel to smelt ${itemName}, you need coal, charcoal, or wood.`,
			);
			if (placedFurnace) await collectBlock(bot, "furnace", 1);
			return false;
		}
		...
		console.log(
			`Added ${put_fuel} ${mc.getItemName(fuel.type)} to furnace fuel.`,
		);
	}
	...
	log(
		bot,
		`Successfully smelted ${itemName}, got ${total} ${mc.getItemName(smelted_item.type)}.`,
	);
	return true;
}
...
export async function defendSelf(bot, range = 9) {
	...
	while (enemy) {
		...
		if (bot.entity.position.distanceTo(enemy.position) <= 2) {
			try {
				bot.pathfinder.setMovements(new pf.Movements(bot));
				let inverted_goal = new pf.goals.GoalInvert(
...
export async function collectBlock(bot, blockType, num = 1, exclude = null) {
	...
	if (blockType.endsWith("ore")) blocktypes.push("deepslate_" + blockType);
	if (blockType === "dirt") blocktypes.push("grass_block");
	if (blockType === "cobblestone") blocktypes.push("stone");
	...
	for (let i = 0; i < num; i++) {
		let blocks = world.getNearestBlocksWhere(
			bot,
			(block) => {
...
export async function placeBlock(
	...
	if (bot.modes.isOn("cheat") && !dontCheat) {
		if (bot.restrict_to_inventory) {
			...
			if (!block) {
				log(
					bot,
					`Cannot place ${blockType}, you are restricted to your current inventory.`,
				);
				return false;
			}
		}
	...
	if (
		targetBlock.name === blockType ||
		(targetBlock.name === "grass_block" && blockType === "dirt")
	) {
	...
	if (
		!dont_move_for.includes(item_name) &&
		(pos.distanceTo(targetBlock.position) < 1.1 ||
			pos_above.distanceTo(targetBlock.position) < 1.1)
	) {
		// too close
...
export async function takeFromChest(bot, itemName, num = -1) {
	...
	let matchingItems = chestContainer
		.containerItems()
...
export async function goToGoal(bot, goal) {
	...
	if (
		(await bot.pathfinder.getPathTo(
			nonDestructiveMovements,
			goal,
			pathfind_timeout,
		).status) === "success"
	) {
...
function startDoorInterval(bot) {
	...
	const doorCheckInterval = setInterval(() => {
		...
		if (stuck_time > 1200) {
			...
			let elevated_positions = positions.map((position) =>
...
export async function goToPosition(bot, x, y, z, min_distance = 2) {
	...
	const checkDigProgress = () => {
		if (bot.targetDigBlock) {
			...
			if (!targetBlock.canHarvest(itemId)) {
				log(
					bot,
					`Pathfinding stopped: Cannot break ${targetBlock.name} with current tools.`,
				);
				bot.pathfinder.stop();
				bot.stopDigging();
			}
	...
	try {
		...
		if (distance <= min_distance + 1) {
			...
			log(
				bot,
				`Unable to reach ${x}, ${y}, ${z}, you are ${Math.round(distance)} blocks away.`,
			);
			return false;
		}
	} catch (err) {
...
export async function goToNearestBlock(
	...
	if (blockType === "water" || blockType === "lava") {
		let blocks = world.getNearestBlocksWhere(
			bot,
			(block) => block.name === blockType && block.metadata === 0,
			range,
			1,
		);
		if (blocks.length === 0) {
			log(
				bot,
				`Could not find any source ${blockType} in ${range} blocks, looking for uncollectable flowing instead...`,
			);
			blocks = world.getNearestBlocksWhere(
				bot,
				(block) => block.name === blockType,
				range,
				1,
			);
		}
...
export async function followPlayer(bot, username, distance = 4) {
	...
	while (!bot.interrupt_code) {
		...
		const distance_from_player = bot.entity.position.distanceTo(
			player.position,
		);
...
export async function tillAndSow(bot, x, y, z, seedType = null) {
	...
	log(
		bot,
		`Planting ${seedType} at x:${x.toFixed(1)}, y:${y.toFixed(1)}, z:${z.toFixed(1)}.`,
	);
...
async function findAndGoToVillager(bot, id) {
	...
	if (entity.metadata && entity.metadata[16] === 1) {
		log(
			bot,
			"This is either a baby villager or a villager with no job - neither can trade",
		);
		return null;
	}
	...
	if (distance > 4) {
		log(
			bot,
			`Villager is ${distance.toFixed(1)} blocks away, moving closer...`,
		);
		try {
			bot.modes.pause("unstuck");
			...
			log(
				bot,
				"Failed to reach villager - pathfinding error or villager moved",
			);
			console.log(err);
			return null;
		} finally {
...
export async function showVillagerTrades(bot, id) {
	...
	try {
		...
		if (!villager.trades || villager.trades.length === 0) {
			log(
				bot,
				"This villager has no trades available - might be sleeping, a baby, or jobless",
			);
			villager.close();
			return false;
		}
		...
		log(
			bot,
			"Failed to open villager trading interface - they might be sleeping, a baby, or jobless",
		);
...
export async function tradeWithVillager(bot, id, index, count) {
	...
	try {
		...
		if (!trade) {
			log(
				bot,
				`Trade ${index} not found. This villager has ${villager.trades.length} trades available.`,
			);
			villager.close();
			return false;
		}
		...
		const item_2 = trade.inputItem2
			? stringifyItem(bot, trade.inputItem2) + " "
			: "";
		log(
			bot,
			`Trading ${stringifyItem(bot, trade.inputItem1)} ${item_2}for ${stringifyItem(bot, trade.outputItem)}...`,
		);
		...
		if (!hasResources(villager.slots, trade, actualCount)) {
			log(
				bot,
				`Don't have enough resources to execute trade ${index} ${actualCount} time(s)`,
			);
			villager.close();
			return false;
		}
...
function stringifyItem(bot, item) {
	...
	if (item.nbt && item.nbt.value) {
		...
		if (Potion)
			text += ` of ${Potion.value.replace(/_/g, " ").split(":")[1] || "unknown type"}`;
		if (display) text += ` named ${display.value.Name.value}`;
		if (ench || StoredEnchantments) {
			text += ` enchanted with ${(ench || StoredEnchantments).value.value
				.map((e) => {
...
export async function digDown(bot, distance = 10) {
	...
	for (let i = 1; i <= distance; i++) {
		...
		if (
			...
			log(
				bot,
				`Dug down ${i - 1} blocks, but reached ${belowBlock ? belowBlock.name : "(lava/water)"}`,
			);
			return false;
		}
		...
		if (num_fall_blocks > MAX_FALL_BLOCKS) {
			log(
				bot,
				`Dug down ${i - 1} blocks, but reached a drop below the next block.`,
			);
			return false;
		}
...
export async function goToSurface(bot) {
	...
	for (let y = 360; y > -64; y--) {
		// probably not the best way to find the surface but it works
		...
		await goToPosition(
			...
			block.position.z,
			0,
		); // this will probably work most of the time but a custom mining and towering up implementation could be added if needed
...
export async function useToolOn(bot, toolName, targetName) {
	...
	if (targetName === "nothing") {
		...
		if (targetName === "water" || targetName === "lava") {
			...
			let blocks = world.getNearestBlocksWhere(
				bot,
				(block) => block.name === targetName && block.metadata === 0,
				64,
				1,
			);
			if (blocks.length === 0) {
...
export async function useToolOnBlock(bot, toolName, block) {
	...
	const distance =
		toolName === "water_bucket" && block.name !== "lava" ? 1.5 : 2;
	await goToPosition(
		bot,
		block.position.x,
		block.position.y,
	...
	const viewBlocked = () => {
		...
		return (
			...
			blockInView.position.distanceTo(headPos) <
				block.position.distanceTo(headPos)
		);
	};
	const blockInView = bot.blockAtCursor(5);
	if (viewBlocked()) {
		...
		if (viewBlocked()) {
			...
			log(
				bot,
				`Block ${blockInView.name} is in the way, not using ${toolName}.`,
			);
			return false;
		}
	}
...
export async function buildStructure(bot, blueprint) {
	 ...
	 *   - blocks: Array of {x, y, z, block} relative offsets.
	 *   - origin: {x, y, z} relative to bot current position or an absolute position.
	 **/
	const startPos = bot.entity.position.clone().floor();
	const origin = blueprint.origin || { x: 0, y: 0, z: 0 };
	const absOrigin = startPos.offset(origin.x, origin.y, origin.z);
	...
	log(bot, `Building structure at ${absOrigin}...`);

	for (const b of blueprint.blocks) {
		const targetPos = absOrigin.offset(b.x, b.y, b.z);
		...
		if (!placed) {
			log(bot, `Failed to place ${b.block} at ${targetPos}. Continuing...`);
		}
	}
	log(bot, `Structure building finished.`);
	return true;
}
