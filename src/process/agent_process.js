...
const init_agent_path = fileURLToPath(
	new URL("./init_agent.js", import.meta.url),
);
...
export class AgentProcess {
	...
	start(load_memory = false, init_message = null, count_id = 0) {
		...
		agentProcess.on("exit", (code, signal) => {
			...
			console.log(
				`Agent process exited with code ${code} and signal ${signal} (uptime: ${uptime}ms)`,
			);
			this.running = false;
	...
	_scheduleRestart(uptimeMs, load_memory, count_id) {
		...
		console.log(
			`Reconnecting in ${actualDelay}ms (attempt ${this._restartCount})…`,
		);
		setTimeout(() => {
	...
	forceRestart() {
		if (this.running && this.process && !this.process.killed) {
			console.log(
				`Agent process for ${this.name} is still running. Attempting to force restart.`,
			);

			const restartTimeout = setTimeout(() => {
				console.warn(
					`Agent ${this.name} did not stop in time. It might be stuck.`,
				);
